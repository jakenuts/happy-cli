...
var types = require('./types-QJg7LeZh.cjs');
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        if (!exitReason) {
          session.client.sendSessionEvent({ type: "message", message: "Process exited unexpectedly" }).catch((err) => types.logger.debug("[Claude] Failed to send exit event:", err));
          continue;
        } else {
          break;
        }
...
const __filename$1 = node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index-CcjVa7OK.cjs', document.baseURI).href)));
      ...
      try {
        const remoteResult = await claudeRemote({
          ...
          onCompletionEvent: (message) => {
            ...
            session.client.sendSessionEvent({ type: "message", message }).catch((err) => types.logger.debug("[Claude] Failed to send completion event:", err));
          },
          onSessionReset: () => {
          ...
          onReady: () => {
            if (!pending && session.queue.size() === 0) {
              session.client.sendSessionEvent({ type: "ready" }).catch((err) => types.logger.debug("[Claude] Failed to send ready event:", err));
        ...
        if (!exitReason && abortController.signal.aborted) {
          session.client.sendSessionEvent({ type: "message", message: "Aborted by user" }).catch((err) => types.logger.debug("[Claude] Failed to send abort event:", err));
        }
      } catch (e) {
...
async function startHappyServer(client) {
  ...
  return {
    ...
    stop: () => {
      types.logger.debug("[happyMCP] Stopping server");
      mcp.close();
      server.close();
    }
  };
...
async function runClaude(credentials, options = {}) {
  ...
  await loop({
    ...
    onModeChange: (newMode) => {
      session.sendSessionEvent({ type: "switch", mode: newMode }).catch((err) => types.logger.debug("[Claude] Failed to send mode change event:", err));
      session.updateAgentState((currentState) => ({
        ...currentState,
...
(async () => {
  ...
  if (subcommand === "doctor") {
    ...
    try {
      const { runCodex } = await Promise.resolve().then(function () { return require('./runCodex-CRpVyA_X.cjs'); });
